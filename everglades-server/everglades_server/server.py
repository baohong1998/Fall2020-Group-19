...
class EvergladesGame:
    ...
    def board_init(self, map_file):
        ...
        for i, in_node in enumerate(self.map_dat['nodes']):
            ...
            node.connection_idxs = np.array(
                node.connection_idxs).astype(np.int)
        ...
        self.p1_node_map = [0, 11, 8, 9, 10, 5,
                            6, 7, 2, 3, 4, 1]  # DemoMap mapping
    ...
    def game_init(self, player_dat):
        ...
        for player in players:
            ...
            for i, gid in enumerate(player_dat[player]['unit_config']):
                ...
                # Input validation
                assert(
                    in_type in self.unit_names), 'Group type not in unit type config file'
    ...
    def game_turn(self, actions):
        ...
        for player in self.team_starts:
            ...
            r, c = action.shape[:2]
            assert(c == 2), 'Did not receive 2 columns for player {}s action'.format(
                player)
            ...
            for gid, nid in action.astype(int):
                ...
                if test1 and test2 and test3:
                    ...
                    #Hack: adjustedID
                    # off by 1
                    adjustedGroupId = self.players[player].groups[gid].groupID + 1
    ...
    def game_end(self):
        ...
        for i, node in enumerate(self.evgMap.nodes):
            ...
            if node.controlState != 0:
                ...
                xer = 2 if np.abs(
                    node.controlState) == node.controlPoints else 1
                points = node.controlPoints if xer == 2 else np.abs(
                    node.controlState)
        ...
        for i, pid in enumerate(self.team_starts):
            for i, group in enumerate(self.players[pid].groups):
                if group.destroyed == False:
                    ...
                    scores[pid] += np.sum([(i.count * i.definition.cost)
    ...
    def debug_state(self):
        ...
        for i, nidx in enumerate(self.map_key2):
            ...
            print('\t% Controlled: {}'.format(
                self.evgMap.nodes[nidx].controlState))
            counts = []
            cnt = 0
            ...
            for gid in counts:
                print('\t\ttype: {}'.format(
                    self.players[0].groups[gid].units[0].unitType))
                print('\t\tavg health: {}'.format(np.average(
                    self.players[0].groups[gid].units[0].unitHealth)))
                print('\t\t{}'.format(
                    self.players[0].groups[gid].units[0].unitHealth))
            ...
            for gid in counts:
                print('\t\ttype: {}'.format(
                    self.players[1].groups[gid].units[0].unitType))
                print('\t\tavg health: {}'.format(np.average(
                    self.players[1].groups[gid].units[0].unitHealth)))
                print('\t\t{}'.format(
                    self.players[1].groups[gid].units[0].unitHealth))
        print()
    ...
    def board_state(self, player_num):
        ...
        """
        assert(isinstance(player_num, (int, float))
               ), '"player_num" was not a number'
    ...
    def player_state(self, player_num):
        ...
        for group in player.groups:
            ...
            state[idx+2] = (health * 1.) / \
                units_alive if (units_alive > 0) else 0
    ...
    def combat(self):
        ...
        for node in self.evgMap.nodes:
            ...
            if len(player_gids) >= 2:
                ...
                all_dmg[node.ID] = {
                    pid: {'groups': [], 'units': [], 'health': []} for pid in self.team_starts}
                ...
                for pid in player_gids:
                    ...
                    for tgt_idx in sorted(infliction[pid].keys()):
                        ...
                        while found == False:
                            if tgt_idx < counts[opp_pid][tgt_group]:
                                ...
                                fort_bns = 1 if (
                                    'DEFEND' in node.resource) else 0
                                ...
                                if (len(nulled_ids[opp_pid][tgt_group]) > 0):
                                    # pdb.set_trace()
                                    tgt_idx -= len(nulled_ids[opp_pid]
                                                   [tgt_group])
                                tgt_unit_idx = np.argwhere(
                                    tgt_unit.unitHealth > 0)[tgt_idx]
                                ...
                                if tgt_unit.unitHealth[tgt_unit_idx] <= 0:
                                    ...
                                    nulled_ids[opp_pid][tgt_group].append(
                                        tgt_idx)
                                    # Disband the group
                                    if tgt_unit.count == 0:
                                        # pdb.set_trace()
                                        self.players[opp_pid].groups[tgt_gid].destroyed = True
                                        pop_idx = node.groups[opp_pid].index(
                                            tgt_gid)
                                        ...
                                        self.output['GROUP_Disband'].append(
                                            outstr)
                                else:
                                    outhealth = tgt_armor * \
                                        (tgt_unit.unitHealth[tgt_unit_idx] / 100.)
                                outhealthstr = '{:.6f}'.format(
                                    float(outhealth))
                                all_dmg[node.ID][opp_pid]['groups'].append(
                                    int(outgroup))
                                all_dmg[node.ID][opp_pid]['units'].append(
                                    int(outunit))
                                all_dmg[node.ID][opp_pid]['health'].append(
                                    float(outhealthstr))
                            else:
                    ...
                    outstr = '{:.6f},{},{},[{}],[{}],[{}]'.format(
                        ...
                        node.ID,
                        ';'.join(str(i)
                                 for i in all_dmg[node.ID][opp_pid]['groups']),
                        ';'.join(str(i)
                                 for i in all_dmg[node.ID][opp_pid]['units']),
                        ';'.join(str(i)
                                 for i in all_dmg[node.ID][opp_pid]['health']),
                    )
            ...
            for group in self.players[player].groups:
                ...
                if not group.destroyed:
                    if group.ready:
                        ...
                        start_idx = int(np.squeeze(
                            np.where(self.map_key1 == group.location)))
                        end_idx = int(np.squeeze(
                            np.where(self.map_key1 == group.travel_destination)))
                        ...
                        if group.distance_remaining <= 0:
                            ...
                            self.evgMap.nodes[start_idx].groups[player].remove(
                                group.groupID)
                            self.evgMap.nodes[end_idx].groups[player].append(
                                group.groupID)
                ...
                if (np.abs(node.controlState) < node.controlPoints) or \
                   (controllers[0] != node.controlledBy):
                    ...
                    if self.current_turn == 0:
                        ...
                        # Build output
                        fullctrl = 'true' if np.abs(
                            node.controlState) >= node.controlPoints else 'false'
                        outstr = '{:.6f},{},{},{:.6f},{}'.format(
                            self.current_turn,
                            node.ID,
                            pid,
    ...
    def output_init(self):
        ...
        # os.umask(oldmask)
        # assert(os.path.isdir(self.dat_dir)
        #        ), 'Could not create telemetry output directory'
        ...
        for pid in self.team_starts:
            ...
            for i, nidx in enumerate(self.map_key2):
                ...
                if node.controlledBy == pid or stationed:
                    ...
                    pcnt = '{:.6f}'.format(
                        100. * node.controlState / node.controlPoints)
                ...
                for nid in opp_k.keys():
                    for dst in opp_k[nid].keys():
                        ...
                        outstr = '{:.6f},{},[{}],[{}],{},{},{}'.format(
                            ...
                            opp_pid,
                            ';'.join(str(i)
                                     for i in opp_k[nid][dst]['unitTypes']),
                            ';'.join(str(i)
                                     for i in opp_k[nid][dst]['unitCount']),
                            status,
                            nid,
                            dst
                        )
    ...
    def write_output(self, sock):
        ...
        #            ), 'Could not create telemetry {} output directory'.format(key)
