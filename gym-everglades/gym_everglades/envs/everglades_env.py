...
class EvergladesEnv(gym.Env):
    ...
    def __init__(self):
        ...
        self.action_space = Tuple((Discrete(self.num_groups), Discrete(
            self.num_nodes + 1)) * self.num_actions_per_turn)
    ...
    def _build_observation_space(self):
        # node loc, class, avg health, in transit, num units rem
        group_low = np.array([1, 0, 0, 0, 0])
        group_high = np.array([self.num_nodes, len(
            self.unit_classes), 100, 1, self.num_units])
        ...
        control_point_portion_low = np.array([0, 0, -100, -1])
        control_point_portion_high = np.array([1, 1, 100, self.num_units])

        control_point_state_low = np.tile(
            control_point_portion_low, self.num_nodes)
        control_point_state_high = np.tile(
            control_point_portion_high, self.num_nodes)

        observation_space = Box(
            low=np.concatenate(
                [[1], control_point_state_low, group_state_low]),
            high=np.concatenate(
                [[self.num_turns + 1], control_point_state_high, group_state_high])
        )
        # pdb.set_trace()
    ...
    def _build_groups(self, player_num):
        ...
        for i in range(self.num_groups):
            ...
            if i == self.num_groups - 1:
                unit_configs[i] = (unit_type, self.num_units -
                                   sum([c[1] for c in unit_configs.values()]))
            else:
